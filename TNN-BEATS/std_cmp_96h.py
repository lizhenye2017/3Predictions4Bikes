...
# 构建时间序列数据
def create_sequences(data, target, input_window=96, output_window=96):
    sequences, labels = [], []
...
# 训练集时间序列
X_train, y_train = create_sequences(scaled_features_train, scaled_target_train, input_window=96, output_window=96)
# 测试集时间序列
X_test, y_test = create_sequences(scaled_features_test, scaled_target_test, input_window=96, output_window=96)
...
output_size = y_train.shape[1]  # 预测未来96小时的cnt
epochs = 100
learning_rate = 1e-3
...
for run in range(5):
    ...
    with torch.no_grad():
        ...
        # 将每轮的预测值加入列表
        predicted_values_all_runs.append(predictions_unscaled)

# 步骤 7: 保存预测结果
joblib.dump(predicted_values_all_runs, '96predictions_all_runs.pkl')  # 保存预测结果
...
# 步骤 10: 保存标准化器
joblib.dump(scaler, 'scaler_96h_std.pkl')
joblib.dump(scaler_target, 'scaler_target_96h_std.pkl')

print("Scalers and predictions saved successfully!")