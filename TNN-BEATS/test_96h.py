...
model = NBeatsModel(input_size=96 * len(['season', 'yr', 'mnth', 'hr', 'holiday', 'weekday', 'workingday',
                    ...
                    output_size=96)  # 根据训练脚本的特征数量调整 input_size
model.load_state_dict(torch.load('nbeats_model_96h.pth'))
model.eval()

scaler = joblib.load('scaler_96h.pkl')
scaler_target = joblib.load('scaler_target_96h.pkl')
...
def plot_predict():
    ...
    time_axis = np.arange(predictions_unscaled.shape[1])  # 96小时时间轴
    ...
    for i, idx in enumerate(sample_indices):
        ...
        # 设置每个子图的标题和标签
        if i == 0:
            plt.title(f"96h's Prediction vs Ground Truth - Samples", fontsize=16)
        if i == 2:
    ...
    # 保存图片，设置dpi为300
    plt.savefig("96h_prediction_vs_ground_truth_samples_subplots.png", dpi=300)
